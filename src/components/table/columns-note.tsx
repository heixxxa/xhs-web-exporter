...
export const columns = [
  ...
  columnHelper.accessor('title', {
    ...
    cell: (info) => {
        ...
        return (
            <div class="max-w-xs">
                <a href={note.note_url} target="_blank" class="font-bold hover:underline block truncate" title={note.title}>
                    {note.title || '(No Title)'}
                </a>
  ...
  columnHelper.accessor('user.nickname', {
    ...
    cell: (info) => (
      <div class="flex items-center gap-2">
        {info.row.original.user.avatar && <img src={info.row.original.user.avatar} class="w-6 h-6 rounded-full" />}
